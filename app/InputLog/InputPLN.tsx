...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbywNXx3JLrpjZcm4Ilfk8ke9YZtu22o9MF9y1pSfbiWn5RLblWSq4mca99eLWf_FJ0vgQ/exec";
...
const InputPLN = () => {
    ...
    const handleSubmit = async () => {
        ...
        if( !lwbp || !wbp || !kvarh || !petugasFinal) {
            return Alert.alert("Harap isi semua kolom yang wajib (*).");
        }
        setLoading(true);
        try {
            const payload = { lwbp, wbp, kvarh, keterangan, petugas: petugasFinal, api_key: "api_key_20251010_4f8c2e9b7a1d4c6e8f3a0b2d9e7c5a1f6b3d8c2e9f0a4b6d7c1e3f9a0b2d4c6"};
            ...
            if (json.status === "success") {
                ...
                setLwbp(""); setWbp(""); setKvarh(""); setKeterangan(""); setPetugas(""); setLainnya(""); setShowLainnyaInput(false);
            } else {
                ...
                <KeyboardAwareScrollView extraScrollHeight={80} enableOnAndroid={true} keyboardOpeningTime={0} scrollEnabled={true} className="flex-1">
                    <ScrollView contentContainerStyle={{ padding: 14 }}>
                        ...
                        <View className="px-6 py-8 bg-white rounded-md shadow-md">
                            <Text className="text-xl font-bold mb-4">Form Pencatatan KWH PLN</Text>

                            <Text className="mb-3">LWBP {!lwbp && <Text className="text-red-500">*</Text>}</Text>
                            <TextInput value={lwbp} onChangeText={setLwbp} placeholder="Masukan tingkat lwbp..." className={`border ${lwbp ? "border-blue-500 bg-blue-100" : "border-gray-300 bg-transparent"} placeholder:text-gray-400 p-3 mb-3 rounded focus:border-blue-500`} />

                            <Text className="mb-3">WBP {!wbp && <Text className="text-red-500">*</Text>}</Text>
                            <TextInput value={wbp} onChangeText={setWbp} placeholder="Masukan tingkat wbp..." className={`border ${wbp ? "border-blue-500 bg-blue-100" : "border-gray-300 bg-transparent"} placeholder:text-gray-400 p-3 mb-3 rounded focus:border-blue-500`} />

                            <Text className="mb-3">Kvarh {!kvarh && <Text className="text-red-500">*</Text>}</Text>
                            <TextInput value={kvarh} onChangeText={setKvarh} placeholder="Masukan tingkat kvarh..." className={`border ${kvarh ? "border-blue-500 bg-blue-100" : "border-gray-300 bg-transparent"} placeholder:text-gray-400 p-3 mb-3 rounded focus:border-blue-500`} />
