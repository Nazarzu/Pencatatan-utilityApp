...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw1Z1kr0knPdjWz1wwKmXJHSAf6OLBiB3W4A6jiiHyMqJZx5noAqmnABcdGiKlXoJiK1w/exec";
...
const InputOksigen = () => {
    ...
    const handleSubmit = async () => {
        ...
        if( !isi || !kosong || !pressure || !petugasFinal) {
            return Alert.alert("Harap isi semua kolom yang wajib (*).");
        }
        setLoading(true);
        try {
            const payload = { isi, kosong, pressure, keterangan, petugas: petugasFinal, api_key: "api_key_20251010_4f8c2e9b7a1d4c6e8f3a0b2d9e7c5a1f6b3d8c2e9f0a4b6d7c1e3f9a0b2d4c6"};
            ...
            if (json.status === "success") {
                ...
                setIsi(""); setKosong(""); setPressure(""); setKeterangan(""); setPetugas(""); setLainnya(""); setShowLainnyaInput(false);
            } else {
                ...
                <KeyboardAwareScrollView extraScrollHeight={80} enableOnAndroid={true} keyboardOpeningTime={0} scrollEnabled={true} className="flex-1">
                    <ScrollView contentContainerStyle={{ padding: 14 }}>
                        ...
                        <View className="px-6 py-8 bg-white rounded-md shadow-md">
                            <Text className="text-xl font-bold mb-4">Form Pencatatan Oksigen</Text>

                            <Text className="mb-3">Stok tabung isi {!isi && <Text className="text-red-500">*</Text>}</Text>
                            <TextInput value={isi} onChangeText={setIsi} placeholder="Masukan stok..." className={`border ${isi ? "border-blue-500 bg-blue-100" : "border-gray-300 bg-transparent"} placeholder:text-gray-400 p-3 mb-3 rounded focus:border-blue-500`} />
