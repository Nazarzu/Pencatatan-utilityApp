...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwO-VdoPA3vBOeKRRqNpc__s9dhnQkXhuZ1MYc0MCkn2Ka9DDMy3Q02wjeSgGxLcP8X/exec";
...
const InputDeepwell = () => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const payload = { tipe, pompa, awal, akhir, keterangan, petugas: petugasFinal, api_key: "api_key_20251010_4f8c2e9b7a1d4c6e8f3a0b2d9e7c5a1f6b3d8c2e9f0a4b6d7c1e3f9a0b2d4c6"};
            ...
            if (json.status === "success") {
                ...
                setTipe(""); setPompa(""); setAwal(""); setAkhir(""); setKeterangan(""); setPetugas(""); setLainnya(""); setShowLainnyaInput(false);
            } else {
    ...
    const opsi2 = ["Deepwell 1", "Deepwell 2", "Deepwell 3"];
                            ...
                            <View className="flex-row justify-between mb-4">
                                {opsi.map((item) => (
                                    <TouchableOpacity key={item} onPress={() => setPompa(item)}  className={`flex-row items-center px-3 py-2 border rounded ${pompa === item ? "border-blue-500 bg-blue-100" : "border-gray-300"}`}>
                                        <View className={`w-4 h-4 mr-2 rounded-full border ${pompa === item ? "bg-blue-500 border-blue-500" : "border-gray-400"}`}/>
                                        <Text>{item}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
