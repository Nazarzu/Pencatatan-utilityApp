...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyzTScys8wM4IeDcvpZ8v8swOXRFek5sM6M1XleQWZ0y0ZQQplgZzdhHCHn8qAqBVqeaA/exec";
...
const InputPDAM = () => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const payload = { status, awal, akhir, keterangan, petugas: petugasFinal, api_key: "api_key_20251010_4f8c2e9b7a1d4c6e8f3a0b2d9e7c5a1f6b3d8c2e9f0a4b6d7c1e3f9a0b2d4c6"};
            ...
            if (json.status === "success") {
                ...
                setStatus(""); setAwal(""); setAkhir(""); setKeterangan(""); setPetugas(""); setLainnya(""); setShowLainnyaInput(false);
            } else {
                ...
                <KeyboardAwareScrollView extraScrollHeight={80} enableOnAndroid={true} keyboardOpeningTime={0} scrollEnabled={true} className="flex-1">
                    <ScrollView contentContainerStyle={{ padding: 14 }}>
                        ...
                        <View className="px-6 py-8 bg-white rounded-md shadow-md">
                            <Text className="text-xl font-bold mb-4">Form Pencatatan PDAM</Text>

                            <Text className="mb-3">Status pompa {!status && <Text className="text-red-500">*</Text>}</Text>
                            ...
                            <View className="flex-row gap-1 items-center mb-3">
                                <Text>Stand meter awal {!awal && <Text className="text-red-500">*</Text>}</Text>
                            </View>
