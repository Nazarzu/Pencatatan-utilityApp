...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyni9xRlsGml3l99wiGw12LPlAkjHwMQ9eLIRosITpuha-DapNzJEESiOg6-yWjHIfD/exec";

const InputCapasitor = () => {
    ...
    const handleSubmit = async () => {
        ...
        if( !status || !cos || !petugasFinal) {
            return Alert.alert("Harap isi semua kolom yang wajib (*).");
        }
        setLoading(true);
        try {
            const payload = { status, cos, keterangan, petugas: petugasFinal, api_key: "api_key_20251010_4f8c2e9b7a1d4c6e8f3a0b2d9e7c5a1f6b3d8c2e9f0a4b6d7c1e3f9a0b2d4c6"};
            ...
            if (json.status === "success") {
                ...
                setStatus(""); setCos(""); setKeterangan(""); setPetugas(""); setLainnya(""); setShowLainnyaInput(false);
            } else {
                ...
                <KeyboardAwareScrollView extraScrollHeight={80} enableOnAndroid={true} keyboardOpeningTime={0} scrollEnabled={true} className="flex-1">
                    <ScrollView contentContainerStyle={{ padding: 14 }}>
                        ...
                        <View className="px-6 py-8 bg-white rounded-md shadow-md">
                            <Text className="text-xl font-bold mb-4">Form Capasitor bank</Text>
                            ...
                            <View className="flex-row gap-1 items-center mb-3">
                                <Text>Cos Phi {!cos && <Text className="text-red-500">*</Text>}</Text>
                                <Text className="text-gray-500 text-xs">(Faktor daya)</Text>
                            </View>
                            <TextInput value={cos} onChangeText={setCos} placeholder="Masukan chos phi..." className={`border ${cos ? "border-blue-500 bg-blue-100" : "border-gray-300 bg-transparent"} placeholder:text-gray-400 p-3 mb-3 rounded focus:border-blue-500`} />
