    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const sheetId = "1HoOzWCJ2JaRPUfC-CLnGN9mtUv0ZYmLIboDg4j9Jm_8";
                const sheetName = "Oksigen";
                ...
                if (!json.table || !json.table.cols) {
                    ...
                    const defaultHeaders = ["Stok Tabung Isi", "Stok Tabung Kosong", "Pressure Gauge","Keterangan", "Petugas", "Timestamp"];
                ...
                const formattedData = rows.map((item: any) => ({
                    Isi: item["Stok Tabung Isi"] ?? "",
                    Kosong: item["Stok Tabung Kosong"] ?? "",
            ...
            <SafeAreaView className="flex-1 p-4 bg-gray-100">
                <ScrollView className="mb-4">
                    ...
                    <View className="px-6 py-8 bg-white  shadow-md rounded-md">
                        <Text className="text-lg font-bold">Data Oksigen</Text>
                        <ScrollView horizontal className="mt-4">
                            <View className="">
                                <View className="flex-row bg-gray-200 rounded-t-md">
                                    <Text className="px-5 py-4 font-medium w-40 text-left">Stok Tabung Isi</Text>
                                    <Text className="px-5 py-4 font-medium w-48 text-left">Stok Tabung Kosong</Text>
                                ...
                                <ScrollView>
                                    {data.length === 0 ? (
                                        ...
                                        <View key={index} className="flex-row border border-t-0 border-gray-100">
                                            <Text className="px-5 py-4 w-40 text-left">{item.Isi}</Text>
                                            <Text className="px-5 py-4 w-48 text-justify">{item.Kosong}</Text>
                                            <Text className="px-5 py-4 w-52 text-justify">{item.Pressure}</Text>
