    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const sheetId = "175iO7N5FgMtjjOHvKmBmlKns4YH1Eh_XXpX40U2Ct5c";
                const sheetName = "Liquid";
                ...
                if (!json.table || !json.table.cols) {
                    ...
                    const defaultHeaders = ["Volume", "Pressure gauge", "Keterangan", "Petugas", "Timestamp"];
            ...
            <SafeAreaView className="flex-1 p-4 bg-gray-100">
                <ScrollView className="mb-4">
                    ...
                    <View className="px-6 py-8 bg-white  shadow-md rounded-md">
                        <Text className="text-lg font-bold">Data Liquid</Text>
                        <ScrollView horizontal className="mt-4">
                            <View className="">
                                <View className="flex-row bg-gray-200 rounded-t-md">
                                    <Text className="px-5 py-4 font-medium w-44 text-left">Volume</Text>
                                    <Text className="px-5 py-4 font-medium w-44 text-left">Pressure gauge</Text>
                                ...
                                <ScrollView>
                                    {data.length === 0 ? (
                                        ...
                                        <View key={index} className="flex-row border border-t-0 border-gray-100">
                                            <Text className="px-5 py-4 w-44 text-justify">{item.Volume}</Text>
                                            <Text className="px-5 py-4 w-44 text-justify">{item.Pressure}</Text>
