    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const sheetId = "1VG4f2nA2YJdR56QgFgwl9pzmxj8SX9n6wHtCZ1rJUWA";
                const sheetName = "Trafo";
                ...
                if (!json.table || !json.table.cols) {
                    ...
                    const defaultHeaders = ["Suhu Ruangan", "Suhu Trafo", "Keterangan", "Petugas", "Timestamp"];
                ...
                const formattedData = rows.map((item: any) => ({
                    Ruangan: item["Suhu Ruangan"] ?? "",
                    Trafo: item["Suhu Trafo"] ?? "",
            ...
            <SafeAreaView className="flex-1 p-4 bg-gray-100">
                <ScrollView className="mb-4">
                    ...
                    <View className="px-6 py-8 bg-white  shadow-md rounded-md">
                        <Text className="text-lg font-bold">Data Ruang Trafo</Text>
                        <ScrollView horizontal className="mt-4">
                            <View className="">
                                <View className="flex-row bg-gray-200 rounded-t-md">
                                    <Text className="px-5 py-4 font-medium w-40 text-left">Suhu Ruangan</Text>
                                    <Text className="px-5 py-4 font-medium w-48 text-left">Suhu Trafo</Text>
                                ...
                                <ScrollView>
                                    {data.length === 0 ? (
                                        ...
                                        <View key={index} className="flex-row border border-t-0 border-gray-100">
                                            <Text className="px-5 py-4 w-40 text-left">{item.Ruangan}</Text>
                                            <Text className="px-5 py-4 w-48 text-justify">{item.Trafo}</Text>
